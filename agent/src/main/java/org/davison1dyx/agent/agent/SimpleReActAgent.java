...
public class SimpleReActAgent {
    public static final String REACT_AGENT_SYSTEM_PROMPT = """
            ...
            ## 工具调用规则（极其重要）
            1. 如果需要调用工具：必须使用 Anthropic 官方 ToolCall 结构，并且 **只能通过工具调用字段输出**。
    ...
    public String callInternal(String conversationId, String question) {
        ...
        while (true) {
            ...
            // 出现result无（模型选择无）,但是results有的情况， AI说是因为模型提供几种答案，自动选择了其中一种，这一种刚好是没有工具调用的。
            Optional<Generation> first = chatResponse.chatResponse().getResults().stream().filter(result -> !result.getOutput().getToolCalls().isEmpty()).findFirst();

            messages.add(builder.toolCalls(first.get().getOutput().getToolCalls()).build());


            first.get().getOutput().getToolCalls().forEach(toolCall -> {
